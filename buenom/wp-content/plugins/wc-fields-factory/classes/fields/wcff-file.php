...
class wcff_field_file extends wcff_field {
	...
	function __construct() {
		...
		/* File upload max size validator */
		add_filter( 'wccpf/upload/validate_max_size', array( $this, 'validate_file_upload_max_size' ), 5, 2 );
		/* File upload filter */
		add_filter( 'wccpf/upload/type=file', array( $this, 'process_file_upload' ) );
		/* File upload mimetype add filter */
		add_filter( 'upload_mimes', array( $this, 'accept_file_types' ) );
    	parent::__construct();
	}
	//add mime type callback
	function accept_file_types(){
	...
	function render_wcff_setup_fields( $type = "wccpf" ) { ob_start(); ?>
		...
		<tr>
			<td class="summary">
				<label for="post_type"><?php _e( 'File Max-size', 'wc-fields-factory' ); ?></label>
				<p class="description"><?php _e( 'Maximum size of file upload enter value in kilobytes.!', 'wc-fields-factory' ); ?></p>
			</td>
			<td>
				<div class="wcff-field-types-meta" data-type="text" data-param="max_file_size">
					<input type="text" id="wcff-field-type-meta-max_file_size" value="" placeholder="<?php _e( 'Enter maximum file upload size', 'wc-fields-factory' ); ?>" />						
				</div>
			</td>
		</tr>
		<tr>
	...
	function render_product_field( $field ) {
		...
		<?php if( has_action('wccpf/before/field/rendering' ) && has_action('wccpf/after/field/rendering' ) ) : ?>
			...
			<input type="file" <?php echo isset( $field['max_file_size'] ) ? 'max-size="'. $field["max_file_size"].'"' : ''; ?> class="wccpf-field" name="<?php echo esc_attr( $field["name"] . $name_index . $multifile_support ); ?>" accept="<?php echo $field["filetypes"]; ?>" <?php echo ( isset( $field["multi_file"] ) && $field["multi_file"] == "yes" ) ? 'multiple="multiple"' : ''; ?> wccpf-type="file" wccpf-pattern="mandatory" wccpf-mandatory="<?php echo $field["required"]; ?>" />
	...
	function validate_file_upload_max_size( $field, $file_size ) {
		$size_ok = true;
		if( isset( $field["max_file_size"] ) && $field["max_file_size"] != ""  ){
			if( round( $field["max_file_size"] ) < round( $file_size / 1024  ) ){
				$size_ok = false;
			}
